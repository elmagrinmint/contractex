...
import { PlotFactoryContract, PlotFactoryInstance } from '../types/truffle-contracts/PlotFactory';
import { PlotRegistryContract } from '../types/truffle-contracts/PlotRegistry';
...
const PlotFactory: PlotFactoryContract = artifacts.require('PlotFactory');
const PlotRegistry: PlotRegistryContract = artifacts.require('PlotRegistry');
...
module.exports = async (deployer: Truffle.Deployer, network: string, accounts: string[]) => {
  if (enabledFeatures().includes('PLOTS')) {
    ...
    for (const plot of DUMMY_DATA) {
      await createPlot(factory, plot);
    }
  }
};

async function createPlot(factory: PlotFactoryInstance, plot: IPlot) {
  ...
  await factory.create(plot.recId, plot.capaKey, plot.owner, ipfsHash);
}
